        ...
        if (t_page->role.level == 1)
        {
            ...
            for (i = 0; i < 512; i ++)
            {
                if (t_page->spt[i] & EPTE_P_MASK)
                {
                    ...
                    pt_ptr[i] = (t_page->spt[i] | EPTE_SVE_BIT | 0x3) & ~0x4;//copy over and RW & NX 
...
static void ana_idt_init (void)
{
    ...
    if (guest_vcpu_paste != 0)
    {
        ...
        ei_shar_arg->msr_kernel_gs_base = guest_vcpu_paste->msr_kernel_gs_base;
        ei_shar_arg->tss_base = guest_vcpu_paste->tss_base;
        DBG("guest fs base: %lx, gs base: %lx, kernel_gs_base: %lx. tss_base: %lx. \n", ei_shar_arg->fs_base, ei_shar_arg->gs_base, ei_shar_arg->msr_kernel_gs_base, ei_shar_arg->tss_base);
    }
    /* / */
    ...
    pack_gate(&s, GATE_INTERRUPT, ana_ve_entry + imee_arg.ana_pf_c_page + UK_OFFSET, 0, 7, __KERNEL_CS);
...
static int t_idt_init(void)
{
    ...
    pack_gate(&s, GATE_INTERRUPT, t_int3_exit + imee_arg.exit_gate_addr + UK_OFFSET, 3, 7, __KERNEL_CS);
    memcpy ((void*)(imee_arg.t_idt_va + 0x2*8*0x3), &s, 0x10);//set up int3 entry in t-IDT 
    DBG ("new int3 t-IDT entry: %lx. \n", *((unsigned long*)(imee_arg.t_idt_va+0x2*8*0x3)));
    
    pack_gate(&s, GATE_INTERRUPT, t_ve_exit + imee_arg.exit_gate_addr + UK_OFFSET, 3, 7, __KERNEL_CS);
    memcpy ((void*)(imee_arg.t_idt_va + 0x2*8*20), &s, 0x10);//set up #VE entry in t-IDT 
    DBG ("new VE t-IDT entry: %lx. \n", *((unsigned long*)(imee_arg.t_idt_va+0x2*8*20)));
    
    pack_gate(&s, GATE_INTERRUPT, t_db_exit + imee_arg.exit_gate_addr + UK_OFFSET, 3, 7, __KERNEL_CS);
    memcpy ((void*)(imee_arg.t_idt_va + 0x2*8*1), &s, 0x10);//set up #DB entry in t-IDT 
